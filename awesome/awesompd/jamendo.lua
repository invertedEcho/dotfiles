...
local function str_interpose(coll, sep)
    if #coll == 0 then return "" end
    local result = coll[1]
    for i = 2, #coll do result = result .. sep .. coll[i] end
    print(result)
    return result
end
...
jamendo.FORMAT_OGG = {
    display = "Ogg Vorbis (q4)",
    short_display = "Ogg",
    value = "ogg2"
}
jamendo.ORDER_RATINGDAILY = {
    display = "Daily rating",
    short_display = "daily rating",
    value = "ratingday_desc"
}
jamendo.ORDER_RATINGWEEKLY = {
    display = "Weekly rating",
    short_display = "weekly rating",
    value = "ratingweek_desc"
}
jamendo.ORDER_RATINGTOTAL = {
    display = "All time rating",
...
jamendo.ORDER_RELEVANCE = {
    display = "None (consecutive)",
    short_display = "none",
...
jamendo.SEARCH_ALBUM = {display = "Album", unit = "album", value = "album_id"}
jamendo.SEARCH_TAG = {display = "Tag", unit = "tag", value = "tag_id"}
jamendo.ALL_FORMATS = {jamendo.FORMAT_MP3, jamendo.FORMAT_OGG}
jamendo.ALL_ORDERS = {
    ORDER_RELEVANCE, ORDER_RANDOM, ORDER_RATINGDAILY, ORDER_RATINGWEEKLY,
    ORDER_RATINGTOTAL
}

jamendo.current_request_table = {
    unit = "track",
    fields = {
        "id", "artist_url", "artist_name", "name", "stream", "album_image",
        "album_name"
    },
    joins = {"track_album", "album_artist"},
    params = {
        streamencoding = jamendo.FORMAT_MP3,
...
local search_template = {
    fields = {"id", "name"},
    joins = {},
    params = {order = ORDER_RELEVANCE, n = 1}
}
...
local function get_default_mp3_stream()
    if not default_mp3_stream or (os.time() - default_mp3_stream.last_checked) >
        3600 then
        local trygetlink = jamendo.perform_request(
                               "echo $(curl -w %{redirect_url} " ..
                                   "'http://api.jamendo.com/get2/stream/track/redirect/" ..
                                   "?streamencoding=" ..
                                   jamendo.FORMAT_MP3.value .. "&id=729304')")
        local _, _, prefix =
            string.find(trygetlink, "stream(%d+)%.jamendo%.com")
...
function jamendo.get_id_from_link(link)
    local _, _, id = string.find(link,
                                 "storage%-new.newjamendo.com/%?trackid=(%d+)")
    return id
end
...
local function get_link_by_id(id)
    ...
    return string.format(
               "http://storage-new.newjamendo.com?trackid=%s&format=mp31&u=0",
               id)
end
...
function jamendo.get_track_by_link(link)
    ...
    if id and jamendo_list[id] then return jamendo_list[id] end
end
...
function jamendo.return_track_table(request_table)
    ...
    for i = 1, #parse_table do
        ...
        parse_table[i].artist_name = string.gsub(parse_table[i].artist_name,
                                                 "\\/", "/")
        parse_table[i].name = string.gsub(parse_table[i].name, "\\/", "/")

        parse_table[i].display_name = parse_table[i].artist_name .. " - " ..
                                          parse_table[i].name
        -- Do Jamendo a favor, extract album_id for the track yourself
        -- from album_image link :)
        local _, _, album_id = string.find(parse_table[i].album_image,
                                           "\\/(%d+)\\/covers")
...
function jamendo.form_request(request_table)
    ...
    if request_table.params and request_table.params ~=
        jamendo.current_request_table.params then
        ...
        for k, v in pairs(request_table.params) do params[k] = v end
    ...
    return string.format(curl_str, string.format(url, f_string, unit, j_string,
                                                 param_string))
end
...
function jamendo.utf8_codes_to_symbols(s)
    ...
    local pattern = string.format("\\u(%s%s%s%s?)", hexnums, hexnums, hexnums,
                                  hexnums)
    ...
    if bus then
        ...
        if header == cache_header then
            for l in bus:lines() do
                local _, _, id, artist_link_name, album_name, album_id,
                      track_name = string.find(l,
                                               "(%d+)-([^-]+)-([^-]+)-(%d+)-(.+)")
                track = {}
                track.id = id
                track.artist_link_name =
                    string.gsub(artist_link_name, '\\_', '-')
...
function jamendo.fetch_album_cover_request(track_id)
    ...
    if not file_exists(file_path) then -- We need to download it  
        ...
        if not track.album_image then -- Wow! We have album_id, but
            ...
            track.album_image = string.format(
                                    "http://imgjam.com/albums/s%s/%s/covers/1.100.jpg",
...
function jamendo.get_album_cover_async(track_id)
    local file_path, fetch_req = jamendo.fetch_album_cover_request(track_id)
    if fetch_req then asyncshell.request(fetch_req) end
end
...
function jamendo.perform_request(request_string)
    ...
    if #response == 0 then
        ...
        if #response ~= 0 then return nil end
    end
    return response
end
