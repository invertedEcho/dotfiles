...
function awesompd.try_load(file)
    if awful.util.file_readable(file) then return file end
end

-- Function for loading modules.
function awesompd.try_require(module)
    if awesompd.try_load(awful.util.getdir("config") .. '/' .. module_path ..
                             module .. ".lua") then
...
function awesompd.get_extended_info(track)
    ...
    if track.album_name then result = result .. "\n" .. track.album_name end
    if track.year then result = result .. "\n" .. track.year end
    return result
end
...
-- returns nil.
local function non_empty(s) if s and s ~= "" then return s end end
...
function awesompd.load_icons(path)
    ...
    awesompd.ICONS.PLAY_PAUSE =
        awesompd.try_load(path .. "/play_pause_icon.png")
...
function awesompd:create()
    ...
    instance.widget:connect_signal("mouse::leave",
                                   function(c) instance:hide_notification() end)
    return instance
end
...
function awesompd:run()
    ...
    if self.album_cover_size > 100 then self.album_cover_size = 100 end
...
function awesompd:register_buttons(buttons)
    ...
    for b = 1, #buttons do
        ...
        if type(buttons[b][2]) == "number" then
            ...
            table.insert(self.global_bindings,
                         awful.key(mods, buttons[b][2], buttons[b][3]))
        end
    end
...
function awesompd:mpcquery(human_readable)
    local result = "mpc -h " .. self.servers[self.current_server].server ..
                       " -p " .. self.servers[self.current_server].port .. " "
    if human_readable then
        return result
    else
        return result ..
                   ' -f "%file%-<>-%name%-<>-%title%-<>-%artist%-<>-%album%" '
    end
end
...
function awesompd:command(com, hook)
    ...
    if hook then hook(self, file) end
    file:close()
end
...
function awesompd:command_read(com, mode)
    mode = mode or "*line"
    self:command(com, function(_, f) result = f:read(mode) end)
    return result
end

function awesompd:command_playpause()
    return function() self:command("toggle", self.update_track) end
end

function awesompd:command_next_track()
    return function() self:command("next", self.update_track) end
end
...
function awesompd:command_stop()
    return function() self:command("stop", self.update_track) end
end

function awesompd:command_play_specific(n)
    return function() self:command("play " .. n, self.update_track) end
end
...
function awesompd:command_load_playlist(name)
    return function()
        self:command("load \"" .. name .. "\"",
                     function() self.recreate_menu = true end)
    end
end
...
function awesompd:command_show_menu()
    return function()
        ...
        if self.recreate_menu then
            local new_menu = {}
            if self.main_menu ~= nil then self.main_menu:hide() end
                ...
                local jamendo_menu = {
                    {
                        "Search by", {
                            ...
                            {
                                "Artist",
                                self:menu_jamendo_search_by(
                                    jamendo.SEARCH_ARTIST)
                            },
                            {
                                "Album",
                                self:menu_jamendo_search_by(jamendo.SEARCH_ALBUM)
                            },
                            {
                                "Tag",
                                self:menu_jamendo_search_by(jamendo.SEARCH_TAG)
                            }
                        }
                    }
                }
...
function awesompd:menu_playback()
    if self.recreate_playback then
        ...
        if self:playing_or_paused() then
            ...
            table.insert(new_menu,
                         {"Stop", self:command_stop(), self.ICONS.STOP})
...
function awesompd:menu_list()
    if self.recreate_list then
        ...
        if self.list_array then
            ...
            local start_num = (self.current_number - 15 > 0) and
                                  self.current_number - 15 or 1
            local end_num = (self.current_number + 15 < total_count) and
                                self.current_number + 15 or total_count
            for i = start_num, end_num do
                table.insert(new_menu, {
                    jamendo.replace_link(self.list_array[i]),
                    self:command_play_specific(i), self.current_number == i and
                        (self.status == self.PLAYING and self.ICONS.PLAY or
                            self.ICONS.PAUSE) or nil
                })
            end
        end
    ...
    if self.recreate_playlists then
        ...
        if #self.playlists_array > 0 then
            for i = 1, #self.playlists_array do
                ...
                submenu[2] = {
                    "Replace current",
                    self:command_replace_playlist(self.playlists_array[i])
                }
        ...
        table.insert(new_menu,
                     {"Refresh", function() self:check_playlists() end})
...
function awesompd:menu_servers()
    if self.recreate_servers then
        ...
        for i = 1, #self.servers do
            table.insert(new_menu, {
                "Server: " .. self.servers[i].server .. ", port: " ..
                    self.servers[i].port, function()
                    self:change_server(i)
                end, self:menu_item_radio(i == self.current_server)
            })
        end
...
function awesompd:menu_jamendo_top()
    return function()
        ...
        if not track_table then
            self:show_notification("Can't connect to Jamendo server",
                                   "Please check your network connection")
        else
            self:add_jamendo_tracks(track_table)
            self:show_notification("Jamendo Top 100 by " ..
                                       jamendo.current_request_table.params
                                           .order.short_display, format(
                                       "Added %s tracks to the playlist",
                                       #track_table))
        end
    end
end
...
function awesompd:menu_jamendo_format()
    if self.recreate_jamendo_formats then
        local setformat = function(format)
            return function()
        ...
        for _, format in pairs(jamendo.ALL_FORMATS) do
            table.insert(new_menu, {
                format.display, setformat(format),
        ...
        self.jamendo_formats_menu = {
            "Format: " ..
                jamendo.current_request_table.params.streamencoding
                    .short_display, new_menu
        }
    end
...
function awesompd:menu_jamendo_browse()
    if self.recreate_jamendo_browse and self.browser and
        self.current_track.unique_name then
        local track = jamendo.get_track_by_link(self.current_track.unique_name)
        local new_menu
        if track then
            local artist_link = "http://www.jamendo.com/artist/" ..
                                    track.artist_link_name
            local album_link = "http://www.jamendo.com/album/" .. track.album_id
            new_menu = {
                {"Artist's page", self:command_open_in_browser(artist_link)},
                {"Album's page", self:command_open_in_browser(album_link)}
            }
...
function awesompd:menu_jamendo_order()
    if self.recreate_jamendo_order then
        ...
        local iscurr = function(o)
            return jamendo.current_request_table.params.order.value == o.value
        end

        local new_menu = {}
        for _, order in pairs(jamendo.ALL_ORDERS) do
            table.insert(new_menu, {
                order.display, setorder(order),
        ...
        local callback = function(s)
            ...
            if result then
                ...
                self:show_notification(format("%s \"%s\" was found",
                                              what.display,
                                              result.search_res.name), format(
                                           "Added %s tracks to the playlist",
                                           track_count))
            else
                self:show_notification("Search failed", format(
                                           "%s \"%s\" was not found",
                                           what.display, s))
            end
        end
        self:display_inputbox("Search music on Jamendo", what.display, callback)
    end
end
...
function awesompd:add_jamendo_tracks(track_table)
    for i = 1, #track_table do
        self:command(
            "add '" .. string.gsub(track_table[i].stream, '\\/', '/') .. "'")
    end
    self.recreate_menu = true
    self.recreate_list = true
end
...
function awesompd:notify_state(state_changed)
    state_array = {
        "Volume: " .. self.state_volume, "Repeat: " .. self.state_repeat,
        "Random: " .. self.state_random, "Single: " .. self.state_single,
...
function awesompd:wrap_output(text)
    return format('<span font="%s" color="%s" background="%s">%s%s%s</span>',
                  ...
                  (text == "" and "" or self.ldecorator),
                  awesompd.protect_string(text),
    ...
    if self.scrolling then
        if self.output_size < utf8.len(text) then
            ...
            if self.scroll_pos + self.output_size - 1 > utf8.len(text) then
                ...
                result = result ..
                             utf8.sub(text, 1,
                                      self.scroll_pos + self.output_size - 1 -
                                          utf8.len(text))
...
function awesompd:notify_connect()
    self:show_notification("Connected",
                           "Connection established to " ..
                               self.servers[self.current_server].server ..
                               " on port " ..
...
function awesompd:update_track(file)
    ...
    if not track_line or string.len(track_line) == 0 then
        ...
        if string.find(track_line, "volume:") or
            string.find(track_line, "Updating DB") then
            ...
            local _, _, new_file, station, title, artist, album = string.find(
                                                                      track_line,
                                                                      "(.*)%-<>%-(.*)%-<>%-(.*)%-<>%-(.*)%-<>%-(.*)")
            ...
            if string.match(new_file, "http://") and
                ...
                not string.match(new_file,
                                 "http://storage%-new%.newjamendo%.com") then
            ...
            if new_file ~= self.current_track.unique_name or force_update then
                ...
                self.recreate_playback = true
                self.recreate_list = true
                self.current_number = tonumber(
                                          self.find_pattern(status_line, "%d+"))
                self:update_widget_text()
                ...
                if self.list_array and self.current_number ~= #self.list_array then
                    ...
                    local next_track = self:command_read(
                                           'playlist -f "%file%" | head -' ..
            ...
            self.track_position, self.track_duration = status_line:match(
                                                           "(%d+:%d+)/(%d+:%d+)")
            local progress =
                self.find_pattern(status_line, "%#%d+/%d+") .. " " ..
                    string.sub(status_line, tmp_pst)
...
function awesompd:smart_update()
    -- Kill any set timers
    if self.smart_update_timer.started then self.smart_update_timer:stop() end
...
function awesompd:run_prompt(welcome, hook)
    awful.prompt.run({prompt = welcome}, self.promptbox[mouse.screen].widget,
                     hook)
end
...
-- used.
function awesompd.protect_string(str, for_menu) return awful.util.escape(str) end
...
function awesompd:init_inputbox()
    ...
    self.inputbox = {wibox = wbox, title = wtbox, prompt = wprompt}
end
...
function awesompd:display_inputbox(title_text, prompt_text, hook)
    if not self.inputbox then self:init_inputbox() end
    ...
    local done_callback = function() self.inputbox.wibox.visible = false end
    self.inputbox.title:set_markup("<b>" .. title_text .. "</b>")
    awful.prompt.run({
        prompt = " " .. prompt_text .. ": ",
        bg_cursor = "#222222"
    }, self.inputbox.prompt.widget, exe_callback, nil, nil, nil, done_callback,
                     nil, nil)
...
function awesompd:try_get_local_cover(current_file)
    if self.mpd_config then
        ...
        local _, _, music_folder = string.find(
                                       self.pread('cat ' .. self.mpd_config ..
                                                      ' | grep -v ^"#" | grep music_directory',
                                                  "*line"),
                                       'music_directory%s+"(.+)"')
        ...
        if covers_table.n > 0 then
            ...
            if covers_table.n > 1 then
                ...
                -- expressions suck:[
                local front_cover = self.pread(
                                        'echo "' .. covers ..
                                            '" | grep -P -i "cover|front|folder|albumart" | head -n 1',
                                        "*line")
                if front_cover then
...
function awesompd:command_toggle() return self:command_playpause() end
