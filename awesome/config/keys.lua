...
keys.desktopbuttons = gears.table
                          .join( -- awful.button({ }, 1, sidepanel.toggle)
...
end), awful.key({modkey, "Shift"}, "d",
                function() awful.spawn(apps.launcherext) end), awful.key(
                                       {modkey, "Shift"}, "c", function()
        awful.spawn(apps.roficalc)
    end), awful.key({modkey}, "p", function() awful.spawn(apps.xrandr) end),
...
end), awful.key({modkey, "Shift"}, "i", function()
    awful.spawn(
        "rofi -show power-menu -modi power-menu:~/.local/bin/rofi-power-menu")
end), awful.key({modkey, "Control"}, "l",
                function() awful.spawn("xsecurelock") end), -- media controls
awful.key({}, "XF86AudioLowerVolume", function()
    awful.spawn.easy_async_with_shell(
        "amixer -D pulse set Master 3%-  ; pamixer --get-volume",
        function(stdout)
            local volume = stdout
            awful.spawn.easy_async_with_shell("pamixer --get-muted",
                                              function(stdout)
                local muted = false
                if stdout == "true" then muted = true end
    ...
    awful.spawn.easy_async_with_shell("pamixer -t ; pamixer --get-muted",
                                      function(stdout)
        local muted = false
        if stdout == "true" then muted = true end
...
keys.clientkeys = gears.table.join(awful.key({modkey}, "f", function(c)
    ...
    c:raise()
                                       ...
                                       {modkey}, "c", awful.placement.centered))
