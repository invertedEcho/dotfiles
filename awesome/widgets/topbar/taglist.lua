...
local markupSelected = function(tag)
	return "<span foreground='#ffffff'>  " .. tag.name .. "  </span>"
end
local markupNormal = function(tag)
	return "<span foreground='#aaaaaa'>  " .. tag.name .. "  </span>"
end
		...
		widget_template = {
			...
			create_callback = function(item, tag, index, _)
				...
				item:connect_signal("mouse::leave", function()
					if item.hasBackup and not tag.selected then
						textbox.markup = item.backup
					end
				end)
			end,
